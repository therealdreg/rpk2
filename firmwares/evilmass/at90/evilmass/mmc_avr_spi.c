...
// modified by Dreg for AT90USB1287
...
#define CS_LOW()        PORT_SPI &= ~(1<<DD_CS) /* Set MMC_CS = low */
#define CS_HIGH()       PORT_SPI |= (1<<DD_CS)  /* Set MMC_CS = high */
...
static BYTE
CardType;           /* Card type flags (b0:MMC, b1:SDv1, b2:SDv2, b3:Block addressing) */
...
void power_on (void)
{
    ...
    // modified by Dreg to power on sd
...
/* Send a data block fast */
static
void xmit_spi_multibyte (
    BYTE byte,  /* Data block to be sent */
    UINT cnt        /* Size of data block */
)
{
    do
...
static
int xmit_datablockbyte (
    BYTE byte,   /* 512 byte data block to be transmitted */
...
)
{
    ...
    xmit_spi_multibyte(byte, 512);          /* Data */
    xchg_spi(0xFF);
...
DSTATUS mmc_disk_initialize (void)
{
    ...
    if (send_cmd(CMD0, 0) == 1)             /* Put the card SPI mode */
    {
        ...
        if (send_cmd(CMD8, 0x1AA) == 1)     /* Is the card SDv2? */
        {
            ...
            if (ocr[2] == 0x01
                    && ocr[3] ==
                    0xAA)               /* The card can work at vdd range of 2.7-3.6V */
            {
                while (Timer1
                        && send_cmd(ACMD41, 1UL <<
                                    30));  /* Wait for leaving idle state (ACMD41 with HCS bit) */
                if (Timer1 && send_cmd(CMD58, 0) == 0)          /* Check CCS bit in the OCR */
                {
                    ...
                    ty = (ocr[0] & 0x40) ? CT_SDC2 | CT_BLOCK :
                         CT_SDC2;    /* Check if the card is SDv2 */
                }
            }
        }
        else                                /* SDv1 or MMCv3 */
...
DRESULT mmc_disk_sector_memset (
    BYTE byte,  /* Pointer to the data to be written */
    LBA_t sector       /* Start sector number (LBA) */
)
{
    DWORD sect = (DWORD)sector;
    ...
    if ((send_cmd(CMD24, sect) == 0) && xmit_datablockbyte(byte, 0xFE))
    {
        count = 0;
    }
    ...
    switch (cmd)
    {
        ...
        case CTRL_TRIM:     /* Erase a block of sectors (used when _USE_TRIM in ffconf.h is 1) */
            ...
            if (send_cmd(CMD32, st) == 0 && send_cmd(CMD33, ed) == 0
                    && send_cmd(CMD38, 0) == 0 && wait_ready(60000))   /* Erase sector block */
            {
