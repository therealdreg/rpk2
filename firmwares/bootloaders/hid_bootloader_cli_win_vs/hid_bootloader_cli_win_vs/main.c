#define VERSION_PRJ_NUMBER 3.0

#define xstr(a) str(a)
#define str(a) #a
#define VERSION_PRJ xstr(VERSION_PRJ_NUMBER)
...
void usage(void)
{
...
#ifdef USE_WIN32
    fprintf(stderr, "\t-g : dont ask press enter to exit\n");
#endif
    ...
    fprintf(stderr,
            "\n<MCU> = atmegaXXuY or at90usbXXXY (ex: atmega32u4 or at90usb1287)");
...
int btld(int argc, char* argv[])
{
    unsigned char buf[260];
    int num, addr, r, first_block = 1, waited = 0;

    puts("\nhid_bootloader_cli v" VERSION_PRJ
         " by Dean Camera, modified by Dreg\n");
...
int teensy_open(void)
{
	...
	if (!libusb_teensy_handle)
		libusb_teensy_handle = open_usb_device(0x6969, 0x9693);
	
	if (!libusb_teensy_handle)
		libusb_teensy_handle = open_usb_device(0x6969, 0x9692);
...
int hard_reboot(void)
{
	...
	if (!rebootor)
		rebootor = open_usb_device(0x6969, 0x9693);
	
	if (!rebootor)
		rebootor = open_usb_device(0x6969, 0x9692);
...
int teensy_open(void)
{
    ...
    if (!win32_teensy_handle)
    {
        ...
        if (!win32_teensy_handle)
        {
            win32_teensy_handle = open_usb_device(0x6969, 0x9693);
            if (!win32_teensy_handle)
            {
                win32_teensy_handle = open_usb_device(0x6969, 0x9692);
            }
        }
    }
...
void attach_callback(void* context, IOReturn r, void* hid_mgr,
                     IOHIDDeviceRef dev)
{
    CFTypeRef type;
...
void detach_callback(void* context, IOReturn r, void* hid_mgr,
                     IOHIDDeviceRef dev)
{
...
{
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, 0,
...
int teensy_open(void)
{
	...
	if (!iokit_teensy_reference)
    {
        iokit_teensy_reference = open_usb_device(0x6969, 0x9693);
    }
	
	if (!iokit_teensy_reference)
    {
        iokit_teensy_reference = open_usb_device(0x6969, 0x9692);
    }
...
int teensy_open(void)
{
	...
	if (uhid_teensy_fd < 0)
		uhid_teensy_fd = open_usb_device(0x6969, 0x9693);
	
	if (uhid_teensy_fd < 0)
		uhid_teensy_fd = open_usb_device(0x6969, 0x9692);
...
int hard_reboot(void)
{
	...
	if (rebootor_fd < 0)
		rebootor_fd = open_usb_device(0x6969, 0x9693);
	
	if (rebootor_fd < 0)
		rebootor_fd = open_usb_device(0x6969, 0x9692);
